...
import { fetchFollowingsActivities, fetchActivitiesOfUser } from "@api/activityApi";
...
export const usePersonnalActivities = (user_id: string) => {
    return useQuery({
        ...
        enabled: !!user_id, // S'assurer que user est défini avant d'exécuter la requête
    })
}

export const useHomeActivities = (user: IUser) => {
    return useQuery({
        queryKey: ["home-activities", user.id],
